...
| `POST /api/manage` (`eject-mode`, alias `gaming-mode`) | Canonical shim for the reserved disable-profile `eject` (092): read = legacy gaming shape, mutation = server-admin toggle with implicit blackout confirm + atomic dual-write during the cutover window. |
