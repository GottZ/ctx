...
| `CTX_WRITES_CONFIRM_TTL` / `CTX_WRITES_CONFIRM_RETENTION` | `600` (s) / `24h` | hot | F6-C6 write-confirmation staging store (`context_pending_writes`, migration 089 — TimescaleDB hypertable, 1h chunks). LLM-path writes (MCP/Chat) get staged; only a hash-selected confirm executes them; REST/CLI stay direct. TWO DECOUPLED knobs: `_TTL` is the expiry clock (staged write must be confirmed within it; `0` = stages never expire — NOT feature-off), `_RETENTION` is the eviction window (chunk-drop of old stages, duration suffix `h/d/w/m/y`; `0` = keep forever). The confirm gate itself ships with the later per-key capability wave |
