...
| `ctx types [list\|get <name>]` | Block-type registry (read-only, **any valid key**): list the types visible to your key (shipped `_global` set + your tenant's overlay, each with a `source` badge) or inspect one type's policy config. TTY: table, pipe: JSON; a 404 (unknown/foreign type) exits 1 |
