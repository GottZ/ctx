...
| `ctx gaming [on\|off]` | Eject toggle (legacy command name): drop the GPU-host backends from every chain so the GPU is free (CPU/external stay in as failover). No arg = status (any valid key); `on`/`off` need an admin key. Since U01-W5 it toggles the reserved `eject` disable-profile (092) — persists as that profile row, survives a restart and hits the next chain without one (the legacy `gaming.active` settings key was retired) |
