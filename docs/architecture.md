...
A block of a type whose `retrieval.policy` is `aggregate-to-parent` (resolved from the registry via `Set.AggregateTypes`) is not delivered as itself — it is folded onto its structural parent (`parent_id`). The canonical use is **comment → issue**: the comment passage is the real hit, but the response carries the issue identity, so a query surfaces one issue rather than a scatter of its comments. The fold is a Go post-pass in the query handler, placed **after graph expansion and before the sensitivity annotation** (so a hydrated parent flows through the existing sensitivity/rerank/supersedes/`filterSuperseded` machinery — a placement literally at `filterSuperseded` would leave the hydrated parent's sensitivity zero-valued = credentials = a silent over-block). It is a **zero-DB no-op when no aggregating type is registered** (the current corpus — `comment` ships `excluded`), which keeps `eval.sh` byte-identical.
...
The issue-specific response contract (the `matched_comment` annotation, ×2 over-fetch and per-parent candidate cap at 10k+ comments/repo) is the Achse-02 wave I-E, not the generic mechanism.
