...
# fällt T07 auf exakt die alte Zähl-Probe zurück (51 Tabellen / 41 Spalten
...
# 51/41 gilt ab Migration 124 (graph_cluster_topic; davor 50/41 ab Mig 118,
...
# das nicht. Gegen das stille Vergessen steht seit Migration 124 ein Gate:
# store/cluster_topic_schema_integration_test.go zählt dieselben zwei Werte
...
t07_count_fallback() {
  ...
  if [[ "$tc" == "$T07_EXPECT_TABLES" ]] && [[ "$col" == "$T07_EXPECT_COLUMNS" ]]; then
    ...
    fail "$T" "Zähl-Fallback (Prä-Deploy, $reason): expected $T07_EXPECT_TABLES tables + $T07_EXPECT_COLUMNS columns (Stand Mig 124), got tables=$tc columns=$col"
  fi
}
...
if $WITH_OLLAMA; then
  ...
  # T17 MCP_TOOLS_LIST — /mcp endpoint returns all 12 registered tools
  ...
  # guard_list/guard_resolve aus mcp_guard.go; Registrierung ist
  ...
  if [[ "$tool_count" == "12" ]]; then
    ...
    fail "$T" "expected 12 tools, got: $tool_count (resp: ${mcp_resp:0:200})"
  fi
