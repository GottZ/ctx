...
**Compose gap:** an env var only reaches the container if the `docker-compose.yml` `environment:` block declares it. Fifteen parsed keys are deliberately *not* declared there (`CTX_DREAM_IDLE_WAIT`, `CTX_DREAM_PARALLELISM`, the six `CTX_DREAM_BACKOFF_*`, `CTX_PROMPT_VERSION`, `CONTEXT_DB_SSLMODE`, and the five `CTX_DREAM_EMBED_*` — the latter Bootstrap-only since the backend pool and dedup'd onto the embed row when identical, see above) — setting them in `.env` alone does nothing. The boot dump makes this visible: a var that never arrived shows `"default"` as its source. To use one, add it to the compose `environment:` block (or set a `context_settings` override).
