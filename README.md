...
- **Dream Mode** — continuous autonomous cross-referencing with dual-model support (v6 prompt for qwen3.6:27b non-thinking sampler with explicit causal anti-pattern clauses, dream pipeline version 4), adaptive cooldown, supersedes detection, temporal validation, hard-cap of 5 links per cycle with type-diversity tie-break, and runtime mode control (on/throttled/off via API). Throttled mode pauses between GPU-intensive steps for thermal management. Config: `CTX_DREAM_IDLE_WAIT` (seconds, default 20)
