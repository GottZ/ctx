...
| `CTX_RERANK_ENABLED` / `_HOST` / `_*` | `false` | Post-RRF rerank (default off, fail-open). `_HOST` empty → LLM-as-judge on the chat model; set (e.g. `http://ctx-rerank:8082`) → local **bge-reranker-v2-m3** cross-encoder sidecar (Wave 2, cohere-style `/v1/rerank`, all-local/$0). Knobs: `_MODEL` / `_MAX_DOCS` (default 50; CPU ≈1s/doc, latency not gated) / `_BLEND_WEIGHT` (1.0 = pure cross-encoder order, lower mixes RRF back in) / `_API_KEY`. See `docker-compose.yml` for the sidecar service. |
