...
**Multi-Tenant (released seit v4.0.0, 2026-06-28):** Modell C (3 Ebenen Tenant/Scope/Block; `scope` = Daten-Diskriminator VARCHAR(50), KEIN `tenant_id` auf Daten-Tabellen) — vollständig auf `root`, kein Feature-Branch mehr (`feat/multi-tenant` ist eine eingefrorene ältere Kopie derselben Historie, 0 Commits ahead of `root`; Spitze `52b99b0` ist in v4.0.0 enthalten). Schema lebt seit dem Migrations-Fold in `go/migrations/113_baseline.sql` (niedrigste Migrationsdatei im Verzeichnis) — `context_tenants` :5777, `context_tenant_scopes` :5797, `context_tenant_grants` :6118, `context_tenant_quota` :6282, `context_block_grants` :6478; die früheren Einzeldateien 058–068 gibt es nicht mehr. OAuth/SSO wurde danach separat gebaut und ist seit v4.12.0 (2026-07-10) auf `root` (`context_oauth_clients`/`_codes`/`_providers`, `context_web_sessions`; Plan `.project/plan-oauth-2026-07-09/`) — der alte Merker „tenant-OAuth deferred" gilt nicht mehr. Quirk: `--shared` schreibt in den shared-Layer des Default-Tenants, NICHT cross-tenant (`go/internal/cli/commands.go:236`, `go/internal/handler/context_store.go:303`). Bau-Stand + Sicherheits-Audit kanonisch in ctx (`ctx query "ctx multi-tenant Bau-Stand"`); `.project/plan-multitenant-2026-06-14/` und Decision-Board `gottz.de/decision.html` sind historische Planungsdokumente.
