...
export function renderCoverage(cs: readonly PageContract[] = contracts, pending: readonly PendingContract[] = pendingContracts): string {
  ...
  lines.push('')
  lines.push('## Flake-Politik, Quarantäne & Budgets (design 06 §5.4/§6.3)')
  lines.push('')
  lines.push('**Flake-Prozess-Regel:** Retries sind deklarierter Infra-Schutz (CI: 1, lokal: 0), kein')
  lines.push('Flake-Teppich. Jeder `flaky`-Status (Test bestand erst im Retry) erzeugt eine sichtbare')
  lines.push('CI-Annotation (`.github/scripts/flake-annotations.sh`) und fließt in die nightly Trend-Zeile')
  lines.push('(`.github/scripts/e2e-trend.sh`). **Ein Test, der zweimal in 14 Tagen flaky war, MUSS in')
  lines.push('Quarantäne oder gefixt werden** — nicht optional.')
  lines.push('')
  lines.push('**Quarantäne:** Eintrag in `e2e/quarantine.json` (Pflicht: `issue` + `since`) + `@quarantine`-')
  lines.push('Tag am Test. Das PR-Gate schließt getaggte Tests aus (config `grepInvert`), nightly führt sie')
  lines.push('WEITER aus (`CTX_E2E_QUARANTINE=1` — beobachtet, nicht vergessen). Harter Deckel > 5 ⇒ rot;')
  lines.push('Tag↔Ledger-Bijektion 1:1 (Tag ohne Eintrag ⇒ rot „untracked", Eintrag ohne Tag ⇒ rot „stale") —')
  lines.push('erzwungen in `e2e/contract/quarantine.test.ts`. Ledger leer = gesunder Default.')
  lines.push('')
  lines.push('**Budgets (kalibriert, `.github/e2e-budget.json`):** e2e-Teilbudget (report.json-Wall) + Job-')
  lines.push('Budget; Überschreitung annotiert, Job > 10 min ⇒ rot. Drei Läufe über dem e2e-Teilbudget ⇒')
  lines.push('Sharding-Aktivierung (in ci.yml vorbereitet, NICHT aktiv — §6.3). **History-Budget (nightly):**')
  lines.push('kumuliertes `__screenshots__`-Blob-Volumen der Git-History; Annotation ab 60 MB, dokumentierter')
  lines.push('Eskalationspfad (Orphan-Branch/LFS) ab 150 MB — nie Auto-Fail (`.github/scripts/history-budget.sh`).')
  lines.push('')
  lines.push('## Matrix')
  lines.push('')
