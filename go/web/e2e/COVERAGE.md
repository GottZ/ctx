...
| board | `/board` | member | `default`, `empty` | ✓ | ✓ 6 Shots (states×dark/light×1 VP) | ✓ 1 (Desktop; Mobile fällt mit dem Opt-out) | ✓ 4 Scans (dark/light × 2 VP) | ✓ (1440, Tab-Walk) | OPT-OUT | — (role member: guard gated ist nur /admin,/tenant) | — | `board-10k` (Deckel 300 @ `main.content [data-board-card]`) | — (PV10) |
...
- **board**: Deep-Link auf /board rendert die Status-Spalten aus dem Board-Wire (Reihenfolge == Wire-Order == Type-Config), jede mit ihrem Wire-Count; terminale Spalten (registry workflow.terminal) starten eingeklappt, offene zeigen ihre Karten (Klick → /issues/:id).
...
- **board**: Der 390-Column-Pager ist U09 (design 04 §7-U09); U07 liefert nur das Desktop-Board (horizontal scrollend). Mobile-Baseline erst mit dem Pager, sonst friert sie eine Nicht-Pager-Sicht ein.
