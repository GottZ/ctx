...
func NewSynthesizeHandler(pool *pgxpool.Pool, backendPool *backends.Pool, blocktypes *blocktype.Registry) *SynthesizeHandler {
	return &SynthesizeHandler{pool: pool, backendPool: backendPool, blocktypes: blocktypes}
}
...
func (h *SynthesizeHandler) HandleDaily(w http.ResponseWriter, r *http.Request) {
	...
	router := &dream.Router{Pool: h.backendPool, Report: llm.PoolReporter(h.backendPool), Blocktypes: h.blocktypes}
