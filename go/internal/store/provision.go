...
func ProvisionProject(ctx context.Context, pool *pgxpool.Pool, p ProvisionParams) (*ProvisionResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("store: provision begin: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("store: provision idempotency probe: %w", err)
	}
	if existing != nil {
		if err := tx.Commit(ctx); err != nil {
			return nil, fmt.Errorf("store: provision idempotent commit: %w", err)
		}
	...
	if err := tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("store: provision commit: %w", err)
	}
