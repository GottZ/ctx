...
func TestClusterTopicSchema_Integration(t *testing.T) {
	...
	// test.sh:306 färbt ab hier diesen Test rot statt still durchzugehen.
	t.Run("test_sh_table_count_matches_fresh_db", func(t *testing.T) {
		...
		if got != want {
			t.Errorf("fresh DB has %d tables, test.sh T07_EXPECT_TABLES = %d — nachziehen (test.sh:306)", got, want)
		}
