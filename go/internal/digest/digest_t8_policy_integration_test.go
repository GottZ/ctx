...
func TestRunDigest_IncludeFalseType_AbsentFromTopicMap(t *testing.T) {
	...
	if err := digest.RunDigest(ctx, pool, reg, homeScope, homeScope, []string{homeScope}); err != nil {
...
func TestRunDigest_NilRegistry_FailsLoud(t *testing.T) {
	pool := testdb.SetupTestDB(t)
	if err := digest.RunDigest(context.Background(), pool, nil, "private", "private", []string{"private"}); err == nil {
