...
func TestSynthesizeGate_PublicBackendNeverSeesPersonal(t *testing.T) {
	...
	res, err := Synthesize(context.Background(), nil, bpool, nil, backends.GamingState{}, gateSettings(),
...
func TestSynthesizeGate_ZeroValueActsAsCredentials(t *testing.T) {
	...
	_, err := Synthesize(context.Background(), nil, bpool, nil, backends.GamingState{}, gateSettings(),
		backends.SensPersonal, "q", sources, nil, "", "")
	if err == nil {
