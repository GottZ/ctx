...
func TestGraphExpand_CrossScopeHopIsolation_Integration(t *testing.T) {
	...
	visible, err := rrf.GraphExpand(ctx, pool, seed, []string{seedScope, neighborScope}, nil, []string{"knowledge"}, cfg)
	if err != nil {
	...
	leak, err := rrf.GraphExpand(ctx, pool, seed, []string{seedScope}, nil, []string{"knowledge"}, cfg)
	if err != nil {
